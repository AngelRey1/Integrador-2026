        ...
        <form [formGroup]="form" (ngSubmit)="submit()">
          ...
          <div style="margin-bottom: 20px;">
            ...
            <div style="display: flex; gap: 15px;">
              <label style="display: flex; align-items: center; cursor: pointer; padding: 12px 20px; border: 2px solid #e4e9f2; border-radius: 8px; flex: 1; transition: all 0.3s;" [style.border-color]="form.get('rol')?.value === 'CLIENTE' ? '#667eea' : '#e4e9f2'" [style.background]="form.get('rol')?.value === 'CLIENTE' ? '#f0f4ff' : 'white'">
                ...
                <div>
                  ...
                  <small style="color: #7b8a8b; font-size: 0.75rem;">Buscar entrenadores</small>
                </div>
              </label>
              <label style="display: flex; align-items: center; cursor: pointer; padding: 12px 20px; border: 2px solid #e4e9f2; border-radius: 8px; flex: 1; transition: all 0.3s;" [style.border-color]="form.get('rol')?.value === 'ENTRENADOR' ? '#667eea' : '#e4e9f2'" [style.background]="form.get('rol')?.value === 'ENTRENADOR' ? '#f0f4ff' : 'white'">
                ...
                <div>
                  ...
                  <small style="color: #7b8a8b; font-size: 0.75rem;">Ofrecer servicios</small>
                </div>
              </label>
            </div>
            <small style="color: red; font-size: 0.85rem; margin-top: 5px; display: block;" *ngIf="form.get('rol')?.invalid && form.get('rol')?.touched">
...
export class LoginComponent implements OnInit {
  ...
  submit() {
    ...
    this.auth.login(loginData).pipe(
      ...
      error: (error) => {
        // Manejar errores especiales de estado
        ...
        if (error.code === 'ACCOUNT_REJECTED') {
          this.router.navigate(['/auth/solicitud-rechazada']);
          return;
        }
